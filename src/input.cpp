...
void Input::read(PmMessage msg) {
  ...
  // MrMKCS_instance() will always be non-null; inputs are always
  // attached to them.
  unsigned char status = Pm_MessageStatus(msg);
  switch (status) {
  case START:
    MrMKCS_instance()->start_clock();
    break;
  case CONTINUE:
    MrMKCS_instance()->continue_clock();
    break;
  case STOP:
    MrMKCS_instance()->stop_clock();
  default:
    MrMKCS_instance()->midi_in(msg);
    break;
  }
}
